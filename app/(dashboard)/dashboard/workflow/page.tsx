...
export default function Workflow() {
    const [IngredientsInFridge , setStory] = useState('');
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!IngredientsInFridge .trim()) {
            setError('Please enter a IngredientsInFridge ');
            return;
        }
        setError('');
        setLoading(true);
        ...
        try {
            ...
            if (response.ok) {
                ...
                setError(data.error || 'An error occurred while fetching the IngredientsInFridge .');
            }
        } catch (err) {
            setError('An error occurred while fetching the IngredientsInFridge .');
        } finally {
            setLoading(false);
        }
    };
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div className="space-y-2">
                            <div className="flex justify-between items-center">
                                <label htmlFor="IngredientsInFridge " className="block text-lg font-medium text-gray-700">
                                    Enter Ingredients From Your Fridge  :
                                </label>
                                <WorkflowHistoryDrawer onSelectHistory={handleSelectHistory} />
                            </div>
                            <input
                                id="IngredientsInFridge "
                                type="text"
                                value={IngredientsInFridge }
